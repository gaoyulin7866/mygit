...
Page({
    ...
    recordStart: function (e) {
      ...
      wx.startRecord({
        success: function (res) {
          ...
          diff = Math.ceil(diff)
          that.setData({
            voice: { url: res.tempFilePath, timeLen: diff }
          });
        },
        fail: function (res) {
          console.log('失败', res)
    ...
    recordEnd: function () {
      ...
      setTimeout(() => {
        if (this.tunnel) {
          var _word = '语音' + _this.data.voice.timeLen +'s'
          this.tunnel.emit('speak', { word: _word });
        }
      },1000);
    },
    playVoicess:function(){
      var _this = this
      wx.playVoice({
        filePath: _this.data.voice.url,
        complete: function () {
          console.log('播放完毕')
        }
      })
